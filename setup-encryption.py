...
sudo dd if=/dev/zero of={image_filename}.img bs=1G count={size_disk_G} iflag=fullblock status=progress
sudo openssl genrsa -out {image_filename}.priv {priv_key_size}
sudo cryptsetup luksFormat --cipher aes-xts-plain64 --key-size {key_size} --key-file {image_filename}.priv --hash sha512 --iter-time 7000 {image_filename}.img
sudo cryptsetup luksOpen --key-file {image_filename}.priv {image_filename}.img crypt-home
...
CONFIG_LINE_CRYPTTAB = "\n{}\n".format("\ncrypt-home          {image_filename}.img        {image_filename}.priv\n".format(image_filename = img_filename))
